...
export const envSchema = z
	.object({
		...
		QUEUE_PREPARE_CONCURRENCY: z.coerce
			.number()
			.int()
			.min(1)
			.max(64)
		...
		QUEUE_SUMMARY_CONCURRENCY: z.coerce
			.number()
			.int()
			.min(1)
			.max(64)
			.default(1),
		QUEUE_FINALIZE_CONCURRENCY: z.coerce
			.number()
			.int()
			.min(1)
			.max(64)
		...
		SEARCH_CHANNEL_AGREEMENT_BOOST: z.coerce
			.number()
			.min(0)
			.max(1)
			.default(0.01),
		SEARCH_GRAPH_MAX_CONTRIBUTION: z.coerce
			.number()
			.min(0)
			.max(1)
			.default(0.03),
		...
		SEARCH_MIN_CHANNEL_AGREEMENT: z.coerce
			...
			.max(10)
			.default(2),
		SEARCH_GRAPH_SEED_LIMIT: z.coerce
			.number()
			.int()
			.min(1)
			.max(100)
