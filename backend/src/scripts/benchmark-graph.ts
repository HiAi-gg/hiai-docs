...
async function runBenchmark(
  ...
  for (const spec of queries) {
    ...
    const recallNoGraph =
      noGraphRecallValues.reduce((a, b) => a + b, 0) / noGraphRecallValues.length;
    ...
    const effectiveK = k;
    for (let r = 0; r < runs; r++) {
      const result = await runSearch(baseUrl, apiKey, spec.text, graphEnabled, effectiveK);
  ...
  const aggRecallWithGraph =
    queryResults.reduce((s, q) => s + q.recallWithGraph, 0) / queryResults.length;
  ...
  return {
    label: graphEnabled ? `graph${boost !== null ? ` (boost=${boost})` : ""}` : "no-graph",
    boost,
...
function printTable(results: QueryResult[]): void {
  ...
  for (const q of results) {
    const queryLabel = q.query.length > 40 ? q.query.slice(0, 37) + "..." : q.query;
    console.log(
...
function printAggregate(agg: BenchmarkRunResult["aggregate"], label: string): void {
  console.log(`\n${ANSI.bold}--- ${label} Aggregate ---${ANSI.reset}`);
  console.log(`  Avg recall@k (no graph):  ${agg.avgRecallNoGraph.toFixed(4)}`);
  console.log(`  Avg recall@k (with graph): ${agg.avgRecallWithGraph.toFixed(4)}`);
  console.log(`  Avg Δ:                     ${colorDelta(agg.avgDelta)}`);
  console.log(`  p50 latency (no graph):    ${agg.p50LatencyNoGraph.toFixed(1)} ms`);
  console.log(`  p50 latency (graph):       ${agg.p50LatencyGraph.toFixed(1)} ms`);
  console.log(`  p95 latency (graph):       ${agg.p95LatencyGraph.toFixed(1)} ms`);
}
...
async function main(): Promise<void> {
  ...
  console.log(`${ANSI.bold}${ANSI.yellow}[!] ${ANSI.reset}${ANSI.bold}GraphRAG Benchmark${ANSI.reset}`);
  console.log(`  ${ANSI.dim}Corpus: ${SYNTHETIC_DOCS.length} synthetic documents`);
  ...
  console.log();
  console.log(
    `${ANSI.yellow}⚠  Prerequisites:${ANSI.reset}`,
  );
  console.log(
  ...
  // Health check
  try {
    const healthResp = await fetch(`${args.baseUrl}/api/admin/health/embeddings`, {
  ...
  if (args.boostRange && args.boostRange.length > 0) {
    console.log(`${ANSI.bold}--- Sensitivity: recall@k vs GRAPH_EXPANSION_BOOST ---${ANSI.reset}\n`);
    ...
    for (const boost of args.boostRange) {
      ...
      for (const spec of QUERIES) {
        ...
        for (let r = 0; r < args.runs; r++) {
          const result = await runSearch(args.baseUrl, args.apiKey, spec.text, false, args.k);
          noGraphLatencies.push(result.latencyMs);
          noGraphRecallValues.push(
    ...
    console.log(`${"Boost".padEnd(10)} | ${"Avg recall (graph)".padEnd(18)} | ${"Δ vs baseline".padEnd(12)} | ${"p50 lat".padEnd(10)} | ${"p95 lat".padEnd(10)}`);
  ...
  console.log(`  ${ANSI.dim}Corpus: ${SYNTHETIC_DOCS.length} docs, ${QUERIES.length} queries`);
  console.log(`  ${ANSI.dim}Avg recall@k (no graph):  ${baseline.aggregate.avgRecallNoGraph.toFixed(4)}`);
  console.log(`  ${ANSI.dim}Avg recall@k (with graph): ${graphResult.aggregate.avgRecallWithGraph.toFixed(4)}`);
  console.log(`  ${ANSI.dim}Avg Δ:                     ${(graphResult.aggregate.avgDelta >= 0 ? "+" : "")}${graphResult.aggregate.avgDelta.toFixed(4)}`);
  console.log(`  ${ANSI.dim}p50 latency (graph):       ${graphResult.aggregate.p50LatencyGraph.toFixed(1)} ms`);
  console.log(`  ${ANSI.dim}p95 latency (graph):       ${graphResult.aggregate.p95LatencyGraph.toFixed(1)} ms${ANSI.reset}`);
}
